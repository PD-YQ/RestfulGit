...
export class TagMySuffixResolve implements Resolve<ITagMySuffix> {
  constructor(private service: TagMySuffixService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<ITagMySuffix> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((tag: HttpResponse<TagMySuffix>) => {
          if (tag.body) {
            return of(tag.body);
          } else {
